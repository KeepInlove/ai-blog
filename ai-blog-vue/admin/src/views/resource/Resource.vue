  ...
  <el-card class="main-card">
    ...
    >
      ...
      <el-table-column prop="requetMethod" label="请求方式">
        <template #default="{row}" v-if="row.requestMethod">
          <el-tag :type="tagType(row.requestMethod)">
            {{ row.requestMethod }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="200">
        <template #default="{row}">
          <el-button
            type="text"
            size="mini"
            @click="openAddResourceModel(row)"
            v-if="row.children"
          >
          ...
          <el-button
            type="text"
            size="mini"
            @click="openEditResourceModel(row)"
          >
...
export default {
  ...
  methods: {
    listResources() {
      getInterfaceList({ keywords: this.keywords }).then(res => {
