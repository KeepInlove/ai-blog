...
import com.ai.blog.strategy.context.SocialLoginStrategyContext;
...
import static com.ai.blog.constant.MQPrefixConst.EMAIL_EXCHANGE;
import static com.ai.blog.constant.RedisPrefixConst.*;
import static com.ai.blog.enums.UserAreaTypeEnum.getUserAreaType;
import static com.ai.blog.util.CommonUtils.checkEmail;
import static com.ai.blog.util.CommonUtils.getRandomCode;
...
public class UserAuthServiceImpl extends ServiceImpl<UserAuthDao, UserAuth> implements UserAuthService {
    ...
    @Override
    public String login(UserVO loginUserVO) {
        ...
        // 1. 通过用户名加载用户信息（包括密码和权限）
        UserDetails userDetails = userDetailsService.loadUserByUsername(loginUserVO.getUsername());
        if (userDetails == null) {
            throw new BizException("用户名或密码错误");
        }

        // 2. 校验密码
        if (!new BCryptPasswordEncoder().matches(loginUserVO.getPassword(), userDetails.getPassword())) {
            throw new BizException("用户名或密码错误");
        }
        UserInfoDTO userInfoDTO = convertToUserInfoDTO(userDetails);
        // 4. 生成Token并存入Redis
        // 5. 返回带Token的用户信息
        return tokenService.createToken(userInfoDTO);
    }
    ...
    /**
     * 将UserDetails转换为UserInfoDTO
     */
    private UserInfoDTO convertToUserInfoDTO(UserDetails userDetails) {
        UserInfoDTO dto = new UserInfoDTO();
        if (userDetails instanceof UserDetailDTO userDetailDTO) {
            dto.setUserInfoId(userDetailDTO.getUserInfoId());
            dto.setUsername(userDetailDTO.getUsername());
            dto.setNickname(userDetailDTO.getNickname());
            dto.setAvatar(userDetailDTO.getAvatar());
            dto.setEmail(userDetailDTO.getEmail());
            dto.setIntro(userDetailDTO.getIntro());
            dto.setWebSite(userDetailDTO.getWebSite());
        } else {
